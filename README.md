# MySQL InnoDB Cluster Setup (3-Node Architecture)

This repository documents the complete setup of a **3-node MySQL InnoDB Cluster** using **Group Replication** on Oracle Linux 9.
...
Edit `sudo vi /etc/hosts` on **all nodes**:
...
```bash
# Run this command in all node
sudo firewall-cmd --permanent --add-port={3306,33060,33061}/tcp
...
```bash
sudo grep 'temporary password' /mysql/dc-node1/log/mysqld.log
```
---
...
```sql
ALTER USER 'root'@'localhost' IDENTIFIED BY 'DIU@123Mainul';
FLUSH PRIVILEGES;
```
...
```javascript
var cluster = dba.createCluster('diuCluster');
```
...
This section allows you to **quickly spin up the 3-node environment locally** using **Vagrant + Libvirt (QEMU)**.
